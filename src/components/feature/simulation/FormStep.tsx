...
export function FormStep({ icon: Icon, title, question, inputProps, submitButtonProps, onBack, onNext, hiddenOnBack }: FormStepProps & Props) {
    ...
    return (
        <div className="bg-card rounded-2xl p-6 shadow-[4px_4px_18px_0px_rgba(0,0,0,0.2)] sm:p-8">
            ...
            <form className="flex flex-col gap-4" onSubmit={handleSubmit}> {/* nova função so pra inserir o preventDefault */}
                <Input {...inputProps} value={inputValue} onChange={e => { inputProps.prefix === 'R$' ? setInputValue(formatCurrencyInput(e.target.value)) : setInputValue(e.target.value) }} />
